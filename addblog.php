...
if (isset($_POST['save']))
{
    ...
    {
        $update = mysqli_query($db, "INSERT INTO `blog`(`name`, `description`, `email`) VALUES ('$name','$description','".$_SESSION['email']."')");
        if ($update)
        {
    ...
    {
        $update = mysqli_query($db, "INSERT INTO `blog`(`name`, `description`, `img`, `email`) VALUES ('$name','$description','$newfilename','".$_SESSION['email']."')");
        if ($update)
        {
            $move = move_uploaded_file($_FILES["img"]["tmp_name"], "img/blogs/" . $newfilename);
            if ($move)
            {
                echo "<script>window.open('blogs.php', '_self')</script>";
            }
