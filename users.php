...
{
    $id = $_GET['status'];
    $sqlgetstatus = mysqli_fetch_array(mysqli_query($db, "SELECT * FROM users WHERE id = '$id'"));
    $statusget = $sqlgetstatus['status'];
    if ($statusget == 'Active')
    {
        $update = mysqli_query($db, "UPDATE users SET status = 'UnVerified' WHERE id = '$id'");
        if ($update)
        {
    ...
    {
        $update = mysqli_query($db, "UPDATE users SET status = 'Active' WHERE id = '$id'");
        if ($update)
        {
            echo "<script>window.open('users.php','_self')</script>";
        }
                    ...
                    <div class="col-xl-12">
                        <div class="white_card card_height_100 mb_30">
                                            ...
                                            <div class="col-lg-8 d-flex justify-content-end">
                                                ...
                                                <div class="serach_field-area theme_bg d-flex align-items-center">
                                                    <div class="search_inner">
                                                        <form action="users.php" enctype="multipart/form-data" method="post">
                            ...
                            <div class="white_card_body ">
                                ...
                                {
                                    $search = $_POST['search'];
                                    $sql = mysqli_query($db, "SELECT * FROM `users` WHERE name LIKE '%$search%' and role = 'User' ORDER BY name ASC");
                                }
                                else
                                {
                                    $sql = mysqli_query($db, "SELECT * FROM `users` WHERE role = 'User' ORDER by name ASC");
                                }
